...
while True:
    ...
    if data:
        ...
        cursor.execute("SELECT Routine.id, Routine.total_time, Routine.active_time, Routine.rest_time,  Exercise.ex_zone, max(Exercise.level) as intensidad FROM Routine, Routine_exercise, Exercise WHERE  Routine.id = Routine_exercise.id_routine AND Routine_exercise.id_ex = Exercise.id GROUP BY Routine.id, Exercise.ex_zone")
